 ...
 * path类：
 * 两翍用法：
 * 1.控制点给值迾为1 （当然也叿以迾为0刿1之间的数，但昿会取控制点前后两点之间的十分位点作为贝忞尔曲线的起点和终点＿
 * 2.全部点都设为0.5除了起点和终点，但是会取控制点前后两点之间的丿点作为贝塞尔曲线的起点和终点（当然也叿以取刿的值效果同理，但是千万不能取大亿0.5的值在此用法下＿
 * @version 5.2
 * @date 2026-6-8
 */
...
#include "APP_Bezier_Curve.h" // 包含贝忞尔曲线相关的头文件
#include "APP_Speedplanner.h" // 包含速度规划器相关的头文仿
#include "APP_tool.h"
...
//        point_last_ = start_point; // 设置上一丿点为起点
//        point_last_ = start_point; // 设置上一丿点为起点
//    }
...
//    void reset(void)
//    {
//        m_phase = S_ACCEL_JERK_UP_PHASE;     // 重置阶濵为加速阶殿
//        point_last_ = bc_.Get_Start_point(); // 重置上一丿点为起点
//        distance_ = 0.0f;                    // 重置距翿
//        t_ = 0.0f;                           // 重置参数 t
//        v_resultant_ = 0.0f;                 // 重置速度
//    }
...
//        end_point_ = end_point;                    // 设置终点
//        point_last_ = start_point;                 // 设置上一丿点为起点
//        bc_.Bezier_Update(start_point, end_point); // 更新贝忞尔曲线
//    }
...
//        end_point_ = end_point;                                   // 设置终点
//        point_last_ = start_point;                                // 设置上一丿点为起点
//        bc_.Bezier_Update(start_point, control_point, end_point); // 更新贝忞尔曲线
//    }
...
//     {
//         m_phase = S_ACCEL_JERK_UP_PHASE;                         // 初忋化阶濵为加速阶殿
//         params.initialSpeed = abs(params.initialSpeed) + 0.001f; // 设置初忋速度
//         params.startPos = 0.0f;                                  // 设置起忋位罿
//         params.targetPos = bc_.Get_len();                        // 设置盿标位罿为曲线长庿
//         sp_.param_reset(params);                                 // 重置速度规划参数
//     }
//     SPhase m_phase = S_FINISHED_PHASE;           // 当前规划所处的阶濿
//     BezierCurve bc_;                             // 贝忞尔曲线对象
//     SShapedPlanner1D sp_;                        // 一绿 S 型速度规划噿
//     float t_ = 0.0f;                             // 贝忞尔曲线参数 t
//     float v_resultant_ = 0.0f;                   // 当前速度
//     float distance_ = 0.0001f;                   // 当前距翿
//     Vector2D v_tangent_ = Vector2D(0.0f, 0.0f);  // 切线向量
//     Vector2D point_last_ = Vector2D(0.0f, 0.0f); // 上一丿炿
...
//     * @brief 添加起忋点
//     * @param point_ 起忋点坐标
//     * @param have_start_angle_ 昿否有起忋迒度
//     * @param start_angle_ 起忋迒度倿
//     * @return true 如果添加成功
//     * @return false 如果添加失败
//     */
...
//     * @param yaw 当前舿向迿
//     * @param target_yaw 输出盿标航向迿
//     * @param normal_error 输出法向诿巿
//     * @param tangent_error 输出切向诿巿
//     * @param normal_vector 输出法向釿
//     * @param tangent_vector 输出切向釿
...
//protected:
//    float err_end = 0.0f; // 朿竿诿巿
//    float dead = 0.04f;
//    Vector2D v_output_ = Vector2D(0.0f, 0.0f);
//    // 旧代砿
//    BezierCurve bezier_curve_list[MAX_CURVE_NUM]; // 储存各路段曲线
//    SShapedPlanner1D sp_;                         // 一绿 S 型速度规划噿
//    float total_len = 0;                          // 跿线总长庿
...
//    /*------------------------------跿径参敿-----------------------------------*/
//    bool have_start_angle = 0; // 昿否迁求达到起忋迒度后再吿势
//    float start_angle = 0;     // 起忋迒度
//    float end_angle = 0;       // 结束角度

//    /*------------------------------过程变量-----------------------------------*/
//    float currnet_target_angle = 0; // 当前盿标迒度

//    uint8_t current_bezier_curve_dx = 0; // 当前跿段忹应曲线的索弿

//    float current_t = 0; // 当前坐标对应当前跿段的t倿

//    float current_finished_len = 0; // 已完成的曲线的总长庿

//    float current_curve_len = 0; // 当前曲线走过的长庿

//private:
//    /*---------------------------------状怿-------------------------------------*/
//    bool is_end = false;   // 昿否开姿
//    bool is_start = false; // 昿否结板

//    bool is_init = false; // 昿否初始化

//    /*----------------------------生成跿径的临时变量-------------------------------------*/
//    Generate_Curve_Status generate_status = GENERATE_WAIT_FIRST_POINT; // 生成曲线的状怿

//    Vector2D point_list[3]; // 生成曲线时临时存放点坐标

//    float last_smoothness; // 生成曲线时临时存放上一丿点的平滑程度
//};
