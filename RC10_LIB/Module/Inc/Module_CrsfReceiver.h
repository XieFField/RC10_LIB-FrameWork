...
typedef struct
{
    ...
    // RadioMaster-POCKET ??????
    uint8_t sw_left;                  // ??3?ο????0=??, 1=??, 2=???????????????
    uint8_t sw_right;                 // ??3?ο????0=??, 1=??, 2=??????????????λ??
    uint8_t sw_sa;                    // SA?????0=??, 1=???2?ο????
    uint8_t sw_sb;                    // SB?????0=??, 1=???2?ο????
    uint8_t sw_sc;                    // SC?????0=??, 1=???2?ο????
...
#define CRSF_DMA_ATTR __attribute__((aligned(32)))
    ...
    // // ????
    // float left_y = 0.0f; //left_y
    // float left_x = 0.0f; //left_x
    // float right_x = 0.0f; //right_x
    // float right_y = 0.0f; //right_y
    
    // // ????/?????
    // uint8_t sw_left_ = 1;      // ?????м?λ??
    // uint8_t sw_right_ = 1;
    // uint8_t sw_sa_ = 0;
    // uint8_t sw_sb_ = 0;
    // uint8_t sw_sc_ = 0;
