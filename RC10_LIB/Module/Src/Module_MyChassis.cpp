...
//    for (int i = 0; i < 4; i++) {
//        wheel_speed_[i] = output[i] / this->wheel_radius;//线速度转化为转速
//    }
//    for (int i = 0; i < 4; i++) {
//        if(wheel_speed_[i]>this->max_wheel_rpm)
//            wheel_speed_[i]=this->max_wheel_rpm;
//        else if(wheel_speed_[i]<-(this->max_wheel_rpm))
//            wheel_speed_[i]=-(this->max_wheel_rpm);
//        if(motors_[i])
...
void MyChassis<WheelCount>::forwardKinematics()
{
    ...
    twist.yaw_rate = wheel_radius_m * (-wheel_rpm[0] + wheel_rpm[1] - wheel_rpm[2] + wheel_rpm[3]) * rpm_to_rad_per_s / (4.0f * sum);
}
*/
