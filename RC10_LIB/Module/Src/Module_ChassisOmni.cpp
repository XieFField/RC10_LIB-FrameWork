...
void Chassis_Onim<WheelCount>::inverseKinematics(const Robot_Twist& twist)
{
...
template <std::size_t WheelCount>
void Chassis_Onim<WheelCount>::forwardKinematics(Robot_Twist& twist)
{
    ...
    if constexpr (WheelCount == 3) {
        ...
        twist.vy = (-wheel_speeds[0] - wheel_speeds[1] + wheel_speeds[2]+ wheel_speeds[3]) / (2.0f*1.41421356f);
        twist.vx = (wheel_speeds[0] - wheel_speeds[1] - wheel_speeds[2] + wheel_speeds[3]) / (2.0f*1.41421356f);
    }
}

template <std::size_t WheelCount>
void Chassis_Onim<WheelCount>::updateKinematics()
{
...
template <std::size_t WheelCount>
Chassis_Onim<WheelCount>::Chassis_Onim(float wheel_radius, float max_wheel_rpm, float chassis_radius)
...
// 显式实例化模板类
template class Chassis_Onim<3>;
template class Chassis_Onim<4>;
