...
OnimDemo<WheelCount>::OnimDemo(float wheel_radius, float max_wheel_rpm, float chassis_radius) : Chassis_Onim<WheelCount>(wheel_radius, max_wheel_rpm, chassis_radius), // 调用�?一�?基类的构造函�?
                                                                                                RtosTask("OnimDemo", 1)                                                // 调用�?二个基类的构造函�?
{
}
...
void OnimDemo<WheelCount>::loop()
{
    ...
    const float w_max = 1.0f; // 最大�?�速度 (rad/s)

    // 遥控器控制逻辑
    // 将PPM信号�?�?�?-1�?1之间的�?
    if (abs(air_joy.LEFT_Y - 1500) < 60)
        air_joy.LEFT_Y = 1500;
    if (abs(air_joy.LEFT_X - 1500) < 60)
        air_joy.LEFT_X = 1500;
    if (abs(air_joy.RIGHT_X - 1500) < 60)
...
template class OnimDemo<4>; // 显式实例�?4�?版本
