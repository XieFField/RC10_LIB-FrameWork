...
protected:
    ...
    {
        constexpr float MAX_ROTATE_ANGLE = 270.0f; //最大累计旋转角度，单位：度
    ...
    /**
     * @brief 安全旋转到指定角度
     * @param target_deg 期望的旋转角度（度）
     * @note 该函数会自动选择最优旋转路径，避免超出最大累计旋转角度限制
     * @param final_deg 实际旋转到的角度（度）
     * @param strategy_used 使用的旋转策略
     */
