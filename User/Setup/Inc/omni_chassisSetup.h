...
private:
    ...
    const float LINESPEED_LIMIT = 10 / 500.f; // ���ٶ�����
    const float YAWSPEED_LIMIT = 1 / 500.f;   // yaw�ٶ�����

    float is_chassis_reverse_ = 1.0f;
    
    RmPocketData_t airjoy_data_; // ң�������ݣ���Χ -1 ~ 1

    Debug_Printf debug_uart = Debug_Printf(&huart8); // ���Դ���
    ...
    int num = 0;
    float tNearest = 0.0f;                // ������ڱ����������ϵĲ���t (0~1)
    float tLookahead = 0.0f;              // ǰ�ӵ��ڱ����������ϵĲ���t (0~1)
    float m_lookaheadDist = 0.4f;         // ǰ�Ӿ��� (��λ: ��)
    float lateralError = 0.0f;            // ������� (������ƫ��·���ľ���)
    float correctspeed = 0.0f;            // ������ĺ����ƫ�ٶȴ�С
    Vector2D nearestPt;                   // ·���Ͼ������������ĵ�
    Vector2D lookaheadPt;                 // ·���ϵ�ǰ�ӵ�
    Vector2D lookaheadTangent;            // ǰ�ӵ㴦�����߷�������
    Vector2D pathEnd;                     // ·���յ�����
    Vector2D corrVelocity = {0.0f, 0.0f}; // ������ĺ����ƫ�ٶ�����
    PID_Position pid_track;               // ѭ���������PID������

    // �Ӿ�PID
    PID_Position pid_vision_x;
    PID_Position pid_vision_y;
    PID_Position pid_vision_yaw;
    Point3D vision_data_ = {0.0f, 0.0f, 0.0f}; // ���ڽ��յ��������

public:
    void UpdateVisionData(Point3D data) { vision_data_ = data; } // �����ڻص�����
};
#endif // __cplusplus
