...
typedef struct
{
    ...
    float spin_skew = 0.08f;   // 旋转位置y轴偏移量
...
typedef struct
{
    Vector2D uphill_pos[2] = {{6.0f - 0.6f, 11.41f}, {0.6f, 11.41f}};
    Vector2D uphill_transitiont_pos[2] = {{6.0f - 2.06f, 11.28f}, {2.06f, 11.28f}};
    Vector2D uphill_transitiont_pos_1[2] = {{6.0f - 2.78f, 10.41f}, {2.78f, 10.41f}};
    ...
    Vector2D R1_pos[3][2] = {{{6.0f - 4.43f, 11.328f}, {4.43f, 11.328f}},
                             {{6.0f - 4.43f, 10.775f}, {4.43f, 10.775f}},
                             {{6.0f - 4.43f, 10.205f}, {4.43f, 10.205f}}};
...
public:
    ...
    bool Get_Arm_Start_flag()
    {
        ...
        temp.y = robot_pos_.y;
        // 读取机械臂触发标志。
        if ((MF_AutoCtrler::isInTargetMap(temp, KFS_point.MF1_Point_, 0.02f) || MF_AutoCtrler::isInTargetMap(temp, KFS_point.MF2_Point_, 0.02f) || MF_AutoCtrler::isInTargetMap(temp, KFS_point.MF3_Point_, 0.02f)) && Arm_Start == true)
        {
            return true;
        }
        else
