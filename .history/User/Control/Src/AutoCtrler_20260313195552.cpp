...
/**
 * @brief 判断当前坐标是否处于目标放格中
 */
bool isInTargetMap(Point2D robotPos, int targetMap)
{
    Point2D map_center = MapNum_RealPos[targetMap - 1];
    float half_cell = 0.6f; // 单元格半宽
    
    if (robotPos.x >= map_center.x - half_cell && robotPos.x <= map_center.x + half_cell &&
        robotPos.y >= map_center.y - half_cell && robotPos.y <= map_center.y + half_cell)
    {
}
